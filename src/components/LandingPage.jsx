...
const LandingPage = ({ handleSearch }) => {
  ...
  const fetchSuggestions = async (query) => {
    ...
    // Fetch more results (e.g., 10) to allow for better sorting
    ...
    try {
      ...
      // Sort results by importance (descending) before slicing
      data.sort((a, b) => b.importance - a.importance);

      // Take the top 5 most important results
      setSuggestions(data.slice(0, 5).map(item => ({
        id: item.place_id,
  ...
  const handleSearchSubmit = async (query) => {
    if (typeof query !== 'string') {
      query.preventDefault && query.preventDefault();
      query = searchTerm;
    }
    if (query.trim()) {
        setSuggestions([]);
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-[calc(100vh-180px)]
                    ...
                    text-white rounded-2xl shadow-xl p-10 mt-4 relative">
        ...
        <div className="w-full max-w-xl relative">
          ...
          >
              <input
                  ...
                  placeholder="Enter city name (e.g., Mumbai, Jaipur...)"
                  autoComplete="off"
                  className="w-full sm:w-auto flex-grow px-5 py-3 text-lg text-gray-800 bg-white/90
              ...
              <button
                  type="submit"
                  className="w-full sm:w-auto px-8 py-3 text-lg font-semibold bg-white text-indigo-600 rounded-lg shadow-md
                            hover:bg-indigo-100 hover:text-indigo-700
                            transition-all duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-indigo-300"
              >
                  Search
              </button>
          </form>
          ...
          {isSuggestionsVisible && (
            <ul
              ref={suggestionsRef}
              className="absolute z-10 w-full left-0 right-0 mt-1 bg-white border border-gray-300 rounded-md shadow-lg max-h-60 overflow-y-auto"
            >
              {isSuggestionsLoading ? (
                <li className="px-4 py-2 text-gray-500 italic">Loading suggestions...</li>
              ) : suggestions.length > 0 ? (
                suggestions.map((suggestion) => (
                  <li
                    key={suggestion.id}
                    onClick={() => handleSuggestionClick(suggestion)}
                    className="px-4 py-2 text-gray-800 hover:bg-indigo-100 cursor-pointer"
                  >
                    {suggestion.name}
                  </li>
                ))
              ) : (
                searchTerm.length >= 3 && <li className="px-4 py-2 text-gray-500 italic">No suggestions found.</li>
              )}
            </ul>
          )}
        </div>
